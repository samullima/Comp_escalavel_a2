...
int main() {
    ...
    for (int num_threads : thread_counts) {
        ...
        DataFrame media = groupby_mean(*df2, "account_id", "amount", tp);
        end = high_resolution_clock::now();
        cout << "[groupby_mean] Tempo: "
             << duration_cast<milliseconds>(end - start).count() << " ms" << endl;
        media.DFtoCSV("output/mean/mean" + to_string(num_threads));

        start = high_resolution_clock::now();
        DataFrame join = join_by_key(media, *df1, "account_id", tp);
        end = high_resolution_clock::now();
        cout << "[join_by_key] Tempo: "
             << duration_cast<milliseconds>(end - start).count() << " ms" << endl;
        join.DFtoCSV("output/join/join" + to_string(num_threads));

        start = high_resolution_clock::now();
        DataFrame resultado = classify_accounts_parallel(join, "A_account_id", "A_mean_amount", "B_current_balance", tp);
        end = high_resolution_clock::now();
        cout << "[classify_accounts_parallel] Tempo: "
             << duration_cast<milliseconds>(end - start).count() << " ms" << endl;
        resultado.DFtoCSV("output/client_classification/classification" + to_string(num_threads));
